...
class FoxFan:
	def __init__(self):
		...
		self.makeDirs('americandad')
		self.makeDirs('clevelandshow')
		self.makeDirs('familyguy')
		self.makeDirs('futurama')
		self.makeDirs('simpsons')
	...
	def listSeries(self, film, season):
		...
		for x in series:
			serieID = int(x[1])
			serieName = x[0]
			uri = sys.argv[0] + '?mode=%s&film=%s&season=%s&series=%s' % ("play", film, season, serieID)
