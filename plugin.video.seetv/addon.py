...
class SeeTV:
	...
	def listChannels(self):
		...
		for c in channels:
			uri = sys.argv[0] + '?mode=play&channel=%s' % c[0]
			item = xbmcgui.ListItem(c[1], thumbnailImage=self.icon)
	...
	def playChannel(self,  channel):
		...
		req = requests.get(self.url+'/get/player/'+channel_num, headers={'Referer': channel, 'X-Requested-With': 'XMLHttpRequest'}, cookies=req.cookies)
		if req.status_code != 200:
			xbmc.executebuiltin("XBMC.Notification(%s,%s,%s)" % ('Error', 'Can\'t get page!', str(10 * 1000)))
			return
		page = req.text
